// Quick test for the simplified enhanced test case output
const fetch = require('node-fetch').default || require('node-fetch');

async function testSimplifiedOutput() {
    console.log('🧪 Testing simplified enhanced test case output...\n');

    const enhanceRequest = {
        title: 'Create a new workspace',
        testCaseSteps: [
            'After signing in, create a new workspace',
            'Enter required info, click Create',
            'Ensure that the getting started guide appears'
        ],
        prd: `
...
- [Run End-to-End Tests & Activity Log](https://learn.microsoft.com/en-us/azure/playwright-testing/tutorial-run-end-to-end-tests)
    ...
    try {
        const response = await fetch('http://localhost:3000/api/enhanceTestCase', {
            method: 'POST',
        ...
        if (response.ok) {
            ...
            console.log('✅ Received simplified enhanced test cases!');
            console.log('📋 Content-Type:', response.headers.get('content-type'));
            console.log('📄 Character count:', textReport.length);
            console.log('\n' + '='.repeat(60));
            console.log('ENHANCED TEST CASES OUTPUT:');
